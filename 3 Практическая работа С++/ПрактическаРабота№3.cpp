...
int menu(int xz) {
	...
	cout << "3. Вычисление введенного с клавиатуры выражения в постфиксной записи." << endl;
	cout << "4. Генерирует несколько (на выбор пользователя) вариантов по польской натации (прямой <префиксная запись> и обратной <постфиксная запись>).\n";
	cout << "5. Выход \n";
	while (true)
	{
...
double postfix()
{
	const int SI = 54; // Размер стека кол-во элементов
	double opr[SI]; // Стек (на основе массива).
	int oper_top = 0; /* Индекс элемента массива,
	...
	while (true)
	{
		...
		if (cin >> op)
		{
			...
			if (strchr("+-*/", op))
			{
				...
				if (oper_top < 2)
				{
					cout << "Ввидите хотябы 2 числа. Нечего считать!\n";
					break;
				}
				...
				if (oper_top == SI)
				{
					cout << "Много инфы переполнение стека\n";
					break;
				}
			}
	...
	if (oper_top == 0) // Стек пуст.
	{
		cout << "Введите хотя-бы 1 элемент. \n";
		return 0.;
	}

	return opr[oper_top - 1];
}
...
int main()
{
	...
	if (lab == 3)
	{
		while (true)
		{
			cout << "3. Вычисление введенного с клавиатуры выражения в постфиксной записи." << endl;
			cout << "####@@@@!!!!$$$$Prakt3SemII%%%%^^^^&&&&****~~~~" << endl;
			cout << " Ввидите формулу в постфиксной форме формат <5 5 + 2 2 + *>: " << endl;
	...
	if (lab == 4) {
		cout << "4. Генерирует несколько (на выбор пользователя) вариантов по обратной польской натации.\n";
		cout << "####@@@@!!!!$$$$Prakt3SemII%%%%^^^^&&&&****~~~~" << endl;
		char in[41], out[41];
		int y, strok = 1, t = 0;
		t = shetSt(strok);
